...
import org.mjh.commonutils.http.JdkHttpUtils;
 ...
 * @date 2023/6/9 9:08
 */
public class JdkHttpUtilsTest {
    @ParameterizedTest
    @CsvSource({"https://www.baidu.com"})
    public void testGet(String urlStr) {
        try {
            String responseBody = JdkHttpUtils.requestGet(urlStr);
    ...
    public void testPost(String urlStr) {
        try {
            String responseBody = JdkHttpUtils.requestPost(urlStr, "");
            System.out.println("responseBody: " + responseBody);
//            Assertions.assertTrue(StringUtils.isNotBlank(responseBody), "响应报文体为空");
        } catch (IOException e) {
            e.printStackTrace();
